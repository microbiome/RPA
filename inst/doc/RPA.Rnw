...
\cite{Cope04}. Certain algorithms, including variants of GC-RMA and FARMS
outperformed RMA. On the other hand, RPA outperformed some widely-used
preprocessing algorithms such as RMA. The differences reflect to some
extent the complexity of probe-level models: FARMS and GC-RMA have
more detailed models for probe effects, whereas RMA is a special case
of the RPA algorithm, obtained when the stochastic noise is assumed
equal between all probes. The results indicate that RPA has a
comparable preprocessing performance with standard preprocessing
algorithms, which supports the validity of the probe performance
estimates in RPA. Note that RPA has not been optimized for
preprocessing purposes as it is primarily targeted at probe
