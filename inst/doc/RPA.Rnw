...
reference array and the other arrays. The reference array can be
selected at random; choice of the reference array does not affect the
relative differences between the samples. Probabilistic formulation
allows incorporation of prior information concerning probe reliability
into gene expression analysis within a principled framework.  The
underlying probabilistic model for probe-level observations is
described in \cite{Lahti10thesis, Lahti11}.

The source code is licensed under FreeBSD license; this is one of the
least restrictive open licenses. Your comments and contributions are
